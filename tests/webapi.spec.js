...
const newPayload = {userEmail:"shafans32@gmail.com",userPassword:"nDFrDdJuCq5.Gqz"}

let token;
test.beforeAll( async()=>
{
    const newRequest = await request.newContext();
    const newLogin = await newRequest.post("https://rahulshettyacademy.com/api/ecom/auth/login",
        {
     ...
     const response = await newLogin.json();
...
await page.reload(); // forces the app to load with injected token
